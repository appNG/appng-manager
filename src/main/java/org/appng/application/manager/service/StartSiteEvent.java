...
public class StartSiteEvent extends Event {
	...
	public void perform(Environment environment, Site site) throws InvalidConfigurationException {
		...
		SiteImpl siteToLoad = platformContext.getBean(CoreService.class).getSiteByName(site.getName());
		platformContext.getBean(InitializerService.class).loadSite(siteToLoad, environment, false, new FieldProcessorImpl("start"));
	}
}
